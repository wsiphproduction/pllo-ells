            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Edit Page Modal</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">Edit Page Modal </h4>
            </div>
        </div>

        <form id="createForm" method="POST" action="{{ route('page-modals.update', $pageModal->id) }}" enctype="multipart/form-data">
            <div class="row row-sm">
                ...
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <input name="name" id="name" value="{{ $pageModal->name }}" required type="text" class="form-control @error('name') is-invalid @enderror">
                        @error('name')
                    ...
                    <div class="form-group">
                        @php
                            $modals = explode(',',$pageModal->pages);
                            $arr_pages = [];
                            foreach($modals as $modal){
                                if($modal != ""){
                                    array_push($arr_pages, $modal);
                                }   
                            }
                        @endphp
                        ...
                        <select class="form-control select2 @error('pages') is-invalid @enderror" multiple="multiple" name="pages[]" id="pages" required>
                            ...
                            @foreach($pages as $page)
                                <option value="{{ $page['name'] }}" @if(in_array($page['name'], $arr_pages)) selected @endif>{{ $page['name'] }}</option>
                            @endforeach
                        </select>
                        @error('pages')
                            <span class="text-danger">{{ $message }}</span>
                ...
                <div class="col-lg-12">
                    <div class="form-group">
                        ...
                        <textarea name="content" id="editor1" rows="10" cols="80">{{ $pageModal->content }}</textarea>
                        @error('content')
                ...
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <div class="custom-control custom-switch @error('status') is-invalid @enderror">
                            <input type="checkbox" class="custom-control-input" name="status" id="customSwitch1" {{ (old("status") == "ON" || $pageModal->status == "Active" ? "checked":"") }}>
                            <label class="custom-control-label" id="label_visibility" for="customSwitch1">{{ucfirst(strtolower($pageModal->status))}}</label>
                        </div>
                    </div>
                </div>

                <div class="col-lg-12 mg-t-30">
                    <input class="btn btn-primary btn-sm btn-uppercase" type="submit" value="Update Page Modal">
