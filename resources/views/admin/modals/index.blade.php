    ...
    <div class="container pd-x-0">
        <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
            <div>
                ...
                <h4 class="mg-b-0 tx-spacing--1">Manage Modals</h4>
            </div>
        </div>
        ...
        <div class="row row-sm">
                ...
                <div class="filter-buttons">
                    <div class="d-md-flex bd-highlight">
                        <div class="bd-highlight mg-r-10 mg-t-10">
                            ...
                            @if (auth()->user()->has_access_to_route('modals.multiple.change.status') || auth()->user()->has_access_to_route('modals.multiple.delete'))
                                <div class="list-search d-inline">
                                    <div class="dropdown d-inline">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            @if (auth()->user()->has_access_to_route('modals.multiple.change.status'))
                                            ...
                                            @endif
                                                ...
                                                @if (auth()->user()->has_access_to_route('modals.multiple.delete'))
                        ...
                        <div class="mg-t-10">
                            @if (auth()->user()->has_access_to_route('page-modals.create'))
                                <a class="btn btn-primary btn-sm mg-b-10" href="{{ route('page-modals.create') }}">Create a Modal</a>
                            @endif
                        </div>
                    </div>
                </div>
                    ...
                    <div class="table-responsive-lg">
                        <table class="table mg-b-0 table-light table-hover" style="table-layout: fixed;word-wrap: break-word;">
                            ...
                            @forelse($modals as $modal)
                                @php
                                    $pages = explode(',',$modal->pages);
                                @endphp
                                <tr id="row{{$modal->id}}">
                                    <th>
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input cb" id="cb{{ $modal->id }}">
                                            <label class="custom-control-label" for="cb{{ $modal->id }}"></label>
                                        </div>
                                    </th>
                                    <td><strong @if($modal->trashed()) style="text-decoration:line-through;" @endif> {{ $modal->name }}</strong></td>
                                    <td>
                                    ...
                                    <td>{{ $modal->status }}</td>
                                    <td>{{ Setting::date_for_listing($modal->updated_at) }}</td>
                                    <td>
                                        @if($modal->trashed())
                                            @if (auth()->user()->has_access_to_route('resources.restore'))
                                                <nav class="nav table-options">
                                                    <a class="nav-link" href="{{route('modal.restore', $modal->id)}}" title="Restore this category"><i data-feather="rotate-ccw"></i></a>
                                                </nav>
                                            @endif
                                        @else
                                        <nav class="nav table-options">
                                            @if (auth()->user()->has_access_to_route('page-modals.edit'))
                                                <a class="nav-link" href="{{ route('page-modals.edit',$modal->id) }}" title="Edit Resources"><i data-feather="edit"></i></a>
                                            @endif

                                            @if (auth()->user()->has_access_to_route('modals.single.delete'))
                                                <a class="nav-link" href="javascript:void(0)" onclick="delete_one_category({{$modal->id}},'{{$modal->name}}')" title="Delete modal"><i data-feather="trash"></i></a>
                                            @endif

                                            @if (auth()->user()->has_access_to_route('modals.change-status'))
                                                <a class="nav-link" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                    <i data-feather="settings"></i>
                                                </a>
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    @if($modal->status == 'Active')
                                                        <a class="dropdown-item" href="{{route('modal.change-status',[$modal->id,'Inactive'])}}" > Inactive</a>
                                                    @else
                                                        <a class="dropdown-item" href="{{route('modal.change-status',[$modal->id,'Active'])}}"> Active</a>
                                                    @endif
                                                </div>
                                            @endif
                                        @endif
                                ...
                                @empty
                                <tr>
                                    <th colspan="6" style="text-align: center;"> <p class="text-danger">No modals found.</p></th>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="col-md-6">
                <div class="mg-t-5">
                    @if ($modals->firstItem() == null)
                        <p class="tx-gray-400 tx-12 d-inline">{{__('common.showing_zero_items')}}</p>
                    @else
                        <p class="tx-gray-400 tx-12 d-inline">Showing {{ $modals->firstItem() }} to {{ $modals->lastItem() }} of {{ $modals->total() }} items</p>
                    @endif
                </div>
            </div>
            <div class="col-md-6">
                <div class="text-md-right float-md-right mg-t-5">
                    <div>
                        {{ $modals->appends((array) $filter)->links() }}
                    </div>
                </div>
            </div>
        </div>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('page-modals.index') }}";
    ...
    <script>
            ...
            if(parseInt(counter) < 1){
                ...
                if(parseInt(counter)>1){ // ask for confirmation when multiple pages was selected
                    ...
                    $('#btnUpdateStatus').on('click', function() {
                        post_form("{{route('modals.multiple.change.status')}}",status,selected_videos);
                    });
                }
                else{
                    post_form("{{route('modals.multiple.change.status')}}",status,selected_videos);
        ...
        function delete_category(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('modals.multiple.delete')}}",'',selected_entries);
                });
            }
        }
        ...
        function delete_one_category(id,page){
            ...
            $('#btnDelete').on('click', function() {
                post_form("{{route('modal.single.delete')}}",'',id);
            });
        }
