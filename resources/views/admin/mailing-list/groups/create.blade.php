    ...
    <div class="container pd-x-0">
        <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
            <div>
                ...
                <h4 class="mg-b-0 tx-spacing--1">Add Mailing Group</h4>
            </div>
        </div>
                ...
                <div class="form-group">
                    ...
                    <select id="subscribers" name="subscribers[]" multiple="multiple" class="form-control @error('subscribers') is-invalid @enderror">
                        @foreach ($subscribers as $subscriber)
                            <option value="{{ $subscriber->id }}" @if (in_array($subscriber->id, old('subscribers', []))) selected @endif>{{ $subscriber->email }}</option>
                        @endforeach
                    </select>
                    @error('subscribers')
                        <span class="text-danger">{{ $message }}</span>
