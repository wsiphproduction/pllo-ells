            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Forward Campaign</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">Forward Campaign</h4>
            </div>
        </div>
        ...
        <form method="post" class="row row-sm" action="{{ route('mailing-list.campaigns.update', $campaign->id) }}">
            ...
            <div class="col-lg-6">
                <div class="form-group">
                    ...
                    <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $campaign->name) }}" readonly required>
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input name="subject" type="text" class="form-control @error('subject') is-invalid @enderror" value="{{ old('subject', $campaign->subject) }}" readonly required>
                    @error('subject')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="recipients" name="recipients[]" class="form-control @error('recipients') is-invalid @enderror" data-style="btn btn-outline-light btn-md btn-block tx-left" multiple>
                        ...
                        @foreach ($subscribers as $subscriber)
                            <option value="{{ $subscriber->id }}" @if (in_array($subscriber->id, old('recipients', []))) selected @endif>
                ...
                <div class="form-group">
                    ...
                    <select id="recipientGroups" name="recipient_groups[]" class="form-control @error('recipient_groups') is-invalid @enderror" data-style="btn btn-outline-light btn-md btn-block tx-left" multiple>
                        ...
                        @foreach ($groups as $group)
                            <option value="{{ $group->id }}" @if (in_array($group->id, old('recipient_groups', []))) selected @endif>
                                {{ $group->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    @endif
                    <textarea name="content" id="cke_editor1" rows="10" cols="80" readonly required>{{ old('content', $campaign->content) }}</textarea>
                    @error('content')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
            ...
            <div class="col-lg-12 mg-t-10 pd-b-40">
                <input class="btn btn-primary btn-sm tx-uppercase tx-semibold" name="submit" type="submit" value="Send">
