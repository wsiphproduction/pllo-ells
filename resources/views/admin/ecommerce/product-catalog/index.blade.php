...
@section('content')
	<div class="container pd-x-0">
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-5">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Manage Catalog</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">Manage Catalog</h4>
            </div>
        </div>
        ...
        <div class="row row-sm">
                ...
                <div class="filter-buttons">
                    <div class="d-md-flex bd-highlight">
                        <div class="bd-highlight mg-r-10 mg-t-10">
                            ...
                            {{-- @if (auth()->user()->has_access_to_route('product-catalog.multiple.change.status') || auth()->user()->has_access_to_route('promo.multiple.delete'))
                            <div class="list-search d-inline">
                                <div class="dropdown d-inline mg-r-10">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        @if (auth()->user()->has_access_to_route('product-catalog.multiple.change.status'))
                        ...
                        <div class="mg-t-7">
                            @if (auth()->user()->has_access_to_route('product-catalog.create'))
                            <a class="btn btn-primary btn-sm mg-b-20" href="{{route('product-catalog.create')}}">Create a Catalog</a>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="table-list mg-b-10">
                    <div class="table-responsive-lg text-nowrap">
                        <table class="table mg-b-0 table-light table-hover" style="width:100%;">
                            ...
                            <tbody>
                            	@forelse($product_catalog_headers as $product_catalog_header)
                            		<tr>
	                                    <th>
                                            &nbsp;
	                                    </th>
                                        <td>
                                            <strong @if($product_catalog_header->trashed()) style="text-decoration:line-through;" @endif> {{ $product_catalog_header->name }}</strong><br>
                                        </td>
	                                    <td>
	                                    	@if($product_catalog_header->status == 1)
	                                    		<span class="badge badge-success">Active</span>
	                                    	@endif

	                                    	@if($product_catalog_header->status == 0)
	                                    		<span class="badge badge-secondary">Inactive</span>
	                                    	@endif
	                                    </td>
	                                    <td>{{ Setting::date_for_listing($product_catalog_header->updated_at) }}</td>
	                                    <td class="text-right">
	                                        @if($product_catalog_header->trashed())
                                                @if (auth()->user()->has_access_to_route('product-catalog.restore'))
                                                    <nav class="nav table-options">
                                                        <a class="nav-link" href="{{ route('product-catalog.restore', $product_catalog_header->id) }}" title="Restore this catalog"><i data-feather="rotate-ccw"></i></a>
                                                    </nav>
                                                @endif
	                                        @else
	                                            <nav class="nav table-options">
	                                                @if (auth()->user()->has_access_to_route('product-catalog.edit'))
	                                                <a class="nav-link" href="{{ route('product-catalog.edit',$product_catalog_header->id) }}" title="Edit Catalog"><i data-feather="edit"></i></a>
	                                                @endif
	                                                @if (auth()->user()->has_access_to_route('promo.single.delete'))
	                                                <a class="nav-link" href="javascript:void(0)" onclick="delete_one_coupon('{{$product_catalog_header->id}}')" title="Delete Catalog"><i data-feather="trash"></i></a>
	                                                @endif
	                                                ...
	                                                @if (auth()->user()->has_access_to_route('promo.change-status'))
	                                                    ...
	                                                    <div class="dropdown-menu dropdown-menu-right">
	                                                        @if($product_catalog_header->status == 1)
	                                                            <a class="dropdown-item" href="{{route('product-catalog.change-status',[$product_catalog_header->id, 0])}}"> Inactive</a>
	                                                        @else
	                                                            <a class="dropdown-item" href="{{route('product-catalog.change-status',[$product_catalog_header->id, 1])}}"> Active</a>
	                                                        @endif
	                                                    </div>
	                                                @endif
	                                            </nav>
                            	...
                            	@empty
                            		<tr><th colspan="9"><center>No catalog found.</center></th></tr>
                            	@endforelse
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="col-md-6">
                <div class="mg-t-5">
                    @if ($product_catalog_headers->firstItem() == null)
                        <p class="tx-gray-400 tx-12 d-inline">{{__('common.showing_zero_items')}}</p>
                    @else
                        <p class="tx-gray-400 tx-12 d-inline">Showing {{ $product_catalog_headers->firstItem() }} to {{ $product_catalog_headers->lastItem() }} of {{ $product_catalog_headers->total() }} items</p>
                    @endif
                </div>
            </div>
            <div class="col-md-6">
                <div class="text-md-right float-md-right mg-t-5">
                    <div>
                        {{ $product_catalog_headers->appends((array) $filter)->links() }}
                    </div>
                </div>
            </div>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('product-catalog.index') }}";
    ...
    <script>
        ...
        function delete_one_coupon(id){
            ...
            $('#btnDelete').on('click', function() {
                post_form("{{route('product-catalog.single.delete')}}",'',id);
            });
        }
            ...
            if(parseInt(counter) < 1){
                ...
                if(parseInt(counter)>1){ // ask for confirmation when multiple pages was selected
                    ...
                    $('#btnUpdateStatus').on('click', function() {
                        post_form("{{route('product-catalog.multiple.change.status')}}",status,selected_coupons);
                    });
                }
                else{
                    post_form("{{route('product-catalog.multiple.change.status')}}",status,selected_coupons);
        ...
        function delete_coupons(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('product-catalog.multiple.delete')}}",'',selected_coupons);
                });
            }
        }
    </script>
