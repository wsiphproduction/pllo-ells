...
<div class="container pd-x-0">
	<div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
		<div>
			...
			<h4 class="mg-b-0 tx-spacing--1">Edit Catalog</h4>
		</div>
	</div>

	<form method="post" action="{{ route('product-catalog.update',$catalog->id) }}" id="catalogForm" autocomplete="off">
		@csrf
		@method('PUT')
		<div class="row row-sm">
			<div class="col-lg-6">
				<div class="form-group">
					...
					<input type="text" id="catalog_name" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') ?? $catalog->name }}" required>
					@error('name')
						...
						<div class="form-group">
							...
							<select class="form-control select2" multiple="multiple" name="product_name[]" id="product_opt">
								...
								@foreach($products as $product)
									@php
										$isSelected = $catalog_details->contains('product_id', $product->id);
									@endphp
									<option @if($isSelected) selected @endif value="{{$product->id}}">{{$product->name}}</option>
								@endforeach
							</select>
						</div>
						...
						<div class="form-group">
							...
							<select class="form-control select2" multiple="multiple" name="product_category[]" id="category_opt">
								...
								@foreach ($categories as $category)
									@php
										$categoryIds = explode(',', $catalog->category_id);
									@endphp
									<option @if(in_array($category->id, $categoryIds)) selected @endif value="{{ $category->id }}">{{ $category->name }}</option>
								@endforeach
							</select>
						</div>
					</div>
			...
			<div class="col-lg-12">
				<div class="form-group">
					...
					<div class="custom-control custom-switch">
						<input type="checkbox" class="custom-control-input" id="enableSwitch1" name="status" {{ (old("status") == "ON" || $catalog->status == 1 ? "checked":"") }}>
						<label class="custom-control-label" for="enableSwitch1" id="label_status">{{$catalog->status == 1 ? 'Active' : 'Inactive'}}</label>
					</div>
				</div>
			</div>
...
<script>
	...
	$(document).ready(function() {
		...
		// Bind change event to product_opt select
		$('#product_opt').change(function() {
			toggleOptions();
		});

		// Bind change event to category_opt select
	...
	function toggleOptions() {
		var productValue = $('#product_opt').val();
		var categoryValue = $('#category_opt').val();

		if (productValue != '') {
			$('#category_opt').prop("disabled", true);
		} else {
			$('#category_opt').prop("disabled", false);
		}

		if (categoryValue != '') {
			$('#product_opt').prop("disabled", true);
		} else {
			$('#product_opt').prop("disabled", false);
		}
	}
