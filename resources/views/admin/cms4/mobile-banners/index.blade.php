                ...
                <div class="filter-buttons mg-b-10">
                    <div class="d-md-flex bd-highlight">
                        <div class="bd-highlight mg-r-10 mg-t-10">
                            ...
                            @if(auth()->user()->has_access_to_route('mobile-albums.destroy_many'))
                                <div class="list-search d-inline">
                                    <div class="dropdown d-inline mg-r-10">
                                        <button class="btn btn-light btn-sm dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                            Actions
                                        </button>
                                        <form id="albumsForm" method="POST" class="d-none" action="{{ route('mobile-albums.destroy_many') }}">
                                            @method('DELETE')
                                            @csrf
                        ...
                        <div class="ml-auto bd-highlight mg-t-10">
                            <form class="form-inline" id="searchForm">
                                ...
                                @if(auth()->user()->has_access_to_route('mobile-albums.edit'))
                                    <a href="{{ route('mobile-albums.edit', 1) }}" class="btn btn-primary btn-sm mg-b-5 mt-lg-0 mt-md-0 mt-sm-0 mt-1">Manage Home Banner</a>
                                @endif
                                @if(auth()->user()->has_access_to_route('mobile-albums.create'))
                                    <a href="{{route('mobile-albums.create')}}" class="btn btn-primary btn-sm mg-b-5 mg-l-5 mt-lg-0 mt-md-0 mt-sm-0 mt-1">Create an Album</a>
                                @endif
                            </form>
                        </div>
                    </div>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('mobile-albums.index') }}";
...
@section('customjs')
    ...
    <script>
        ...
        $('#promptQuickEdit').on('show.bs.modal', function (e) {
            ...
            let formAction = "{{ route('mobile-albums.quick_update', 0) }}".split('/');
            formAction.pop();
        ...
        $('#preview-banner').on('show.bs.modal', function (e) {
            ...
            $.ajax({
                ...
                url: "{{ route('mobile-albums.banners', '') }}/" + albumId,
