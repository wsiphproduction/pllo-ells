			...
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-body">
						...
						<div class="row mt-3">
							<form id="downloadables_form" action="{{ route('downloadables.front.store') }}" method="post" enctype="multipart/form-data">
								@csrf
			...
			<div class="modal-dialog modal-md">
				<div class="modal-content">
					<div class="modal-body">
						...
						<div class="row mt-3">
							<form id="certificates_form" action="{{ route('downloadables.front.store') }}" method="post" enctype="multipart/form-data">
								@csrf
								...
								@foreach($members as $index => $member)
									@if(App\Models\Custom\Event::isUserParticipated($member->id, $event->id))
										...
										<div class="form-group mb-4">
											...
											<!-- Hidden input and file name display -->
											<input type="file" id="file-upload-{{ $index }}" name="file_url[]" class="d-none file-input"
...
@section('pagejs')
	...
	<!-- JavaScript to handle showing file names -->
	{{-- <script>
		document.querySelectorAll('.file-input').forEach(function (input) {
			input.addEventListener('change', function () {
				const fileNameContainer = document.getElementById('file-name-' + this.id.split('-').pop());
				const files = Array.from(this.files);
				fileNameContainer.innerHTML = files.length
