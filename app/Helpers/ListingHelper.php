...
class ListingHelper
{
    ...
    public function advance_search($model, $searchFields, $equalSearchFields)
    {
        ...
        foreach ($equalSearchFields as $fieldName) {
            ...
            } elseif (isset($searchFields['price1'])) {
                $queryBuilder->whereBetween('price', [$searchFields['price1'], $searchFields['price2']]);

                unset($searchFields['price1']);
                unset($searchFields['price2']);
            } elseif (isset($searchFields['brand'])) {
                $queryBuilder->where('brand_id', 'like', '%'.$searchFields['brand'].'%');

                unset($searchFields['brand']);
            } else {
                //dd($searchFields);
